...
let JwtAdminStrategy = class JwtAdminStrategy extends (0, passport_1.PassportStrategy)(passport_jwt_1.Strategy, 'jwt-admin') {
    ...
    constructor(configService) {
        super({
            jwtFromRequest: passport_jwt_1.ExtractJwt.fromExtractors([
                (req) => {
                    if (!req.cookies['jwt']) {
                        throw new common_1.UnauthorizedException('Debe iniciar sesión como adminsitrador.');
                    }
    ...
    async validate(req, payload) {
        if (payload.email != process.env.ADMIN_EMAIL)
            throw new common_1.UnauthorizedException('Solo un administrador puede hacer operaciones en este contexto');
        return payload;
    }
};
